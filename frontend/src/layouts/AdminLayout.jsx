...
export default function AdminLayout() {
  ...
  const menuItems = [
    ...
    { path: '/admin/indicators', label: 'Indicators', icon: '📈' },
    { path: '/admin/strategies', label: 'Strategies', icon: '🎯' },
    { path: '/admin/promo-codes', label: 'Promo Codes', icon: '🎟️' },
    { path: '/admin/users', label: 'Users', icon: '👥' },
    { path: '/admin/transactions', label: 'Transaction History', icon: '💰' },
          ...
          <div className="flex items-center space-x-4">
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              {menuItems.find(item => location.pathname === item.path || 
                                  (item.path !== '/admin' && location.pathname.startsWith(item.path)))?.label || 'Admin Dashboard'}
            </h2>
          </div>
